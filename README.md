...
- **Design iteration:** I used the first Claude Design draft as a starting point rather than exploring several directions, leaving more time for the required flows. I would revisit the visual hierarchy and design decisions with more time.
- **Component styling:** I used shadcn/ui to move quickly and keep components consistent. I would refine some interactions further, especially making hover states more subtle while keeping keyboard focus clear.
- **Scope and verification:** I ran audits against the assignment checklist to identify missing requirements and avoid building blindly. Dark mode and learning tracks are extra features; neither is required to browse courses or track completion.
- **Testing:** I focused automated testing on progress, filters, and storage behavior. I did not add Playwright or Cypress; an end-to-end sign-in-to-progress-persistence test would be my next step.

## Assessment assets

This is a fictional assessment, not an official Better U product. The interface uses a generic learning icon, a locally defined book favicon, and a static CSS background in place of the website logo and landing-page video used in an earlier version. Course thumbnails come from the supplied catalog. The lesson player uses a separate CC0 flower preview from MDN, not Better U footage; completing it is a demonstration, not verified watch-time tracking.
...
- **Browser devtools** — inspected public website assets during exploration. The website logo and landing-page video were subsequently removed from the app because permission to reuse them was unclear.
...
- **GPT-6 / Astra** — larger UI/UX passes, including layout, catalog, lesson player, and accessibility.
...
1. **Audit the brief against the repo before adding features.** I asked the agent to map the current tree to the Definition of Done (auth, catalog filters, debounce, progress, skeletons, empty states) and list only what was still missing. This helped me check scope and identify gaps.
...
Hover states. The model kept styling inner fragments of the profile control, so hover never covered the whole row (avatar and name). Telling it to try again did not help, because it was still guessing at DOM I had not shown it. I identified the Clerk component and gave the model that context so it could target the appropriate appearance settings instead of guessing.
...
1. With permission and an approved demo account, explore Better U’s learner workflow to understand which patterns could improve this dashboard.
